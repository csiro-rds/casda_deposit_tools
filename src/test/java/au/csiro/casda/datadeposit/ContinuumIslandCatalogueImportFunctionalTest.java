...
public class ContinuumIslandCatalogueImportFunctionalTest extends FunctionalTestBase
{
    ...
    public void testContinuumIslandCatalogueDatafileLoadedAndPersistedToDatabase()
    {
        ...
        assertEquals(2, observation.getCataloguesOfType(CatalogueType.CONTINUUM_ISLAND).size());
        Catalogue catalogue = observation.getCataloguesOfType(CatalogueType.CONTINUUM_ISLAND).get(1);
        ...
        assertEquals(null, continuumIsland.getFluxIntErr());
        assertEquals(1.2909, continuumIsland.getFluxPeak(), 1e-5);
        assertEquals(null, continuumIsland.getMeanBackground());
        assertEquals(null, continuumIsland.getBackgroundNoise());
        assertEquals(null, continuumIsland.getMaxResidual());
        assertEquals(null, continuumIsland.getMinResidual());
        assertEquals(null, continuumIsland.getMeanResidual());
        assertEquals(null, continuumIsland.getRmsResidual());
        assertEquals(null, continuumIsland.getStddevResidual());
        ...
        assertEquals(null, continuumIsland.getSolidAngle());
        assertEquals(null, continuumIsland.getBeamArea());
    ...
    @Test
    @Transactional
    public void testNewContinuumIslandCatalogueDatafileLoadedAndPersistedToDatabase()
    {
        String sbid = "12345";
        String depositDir = "src/test/resources/functional_test/";
        ...
        String continuumCatalogueDatafile = "selavy-results.islands-new.xml";
        try
        {
            catalogueCommandLineImporter.run("-catalogue-type", "continuum-island", "-parent-id", sbid,
                    "-catalogue-filename", continuumCatalogueDatafile, "-infile",
        ...
        assertEquals(2, continuumIslandRepository.countByCatalogue(catalogue));
        ...
        assertEquals(157.31, continuumIsland.getPosAng(), 1e-3);
        assertEquals(1.43658, continuumIsland.getFluxInt(), 1e-6);
        assertEquals(0.005, continuumIsland.getFluxIntErr(), 1e-6);
        assertEquals(1.2909, continuumIsland.getFluxPeak(), 1e-5);
        assertEquals(123.0, continuumIsland.getMeanBackground(), 1e-6);
        assertEquals(456.0, continuumIsland.getBackgroundNoise(), 1e-6);
        assertEquals(0.015, continuumIsland.getMaxResidual(), 1e-6);
        assertEquals(0.025, continuumIsland.getMinResidual(), 1e-6);
        assertEquals(0.035, continuumIsland.getMeanResidual(), 1e-6);
        assertEquals(0.045, continuumIsland.getRmsResidual(), 1e-6);
        assertEquals(0.055, continuumIsland.getStddevResidual(), 1e-6);
        assertEquals(2350, continuumIsland.getXMin());
        assertEquals(2361, continuumIsland.getXMax());
        assertEquals(290, continuumIsland.getYMin());
        assertEquals(308, continuumIsland.getYMax());
        assertEquals(178, continuumIsland.getNumPixels());
        assertEquals(99.12, continuumIsland.getSolidAngle(), 1e-2);
        assertEquals(17.89, continuumIsland.getBeamArea(), 1e-2);
        assertEquals(2355.73, continuumIsland.getXAve(), 1e-4);
        assertEquals(299.16, continuumIsland.getYAve(), 1e-4);
        assertEquals(2355.49, continuumIsland.getXCen(), 1e-4);
        assertEquals(300.12, continuumIsland.getYCen(), 1e-4);
